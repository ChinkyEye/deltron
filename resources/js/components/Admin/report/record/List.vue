...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      pagechange(){
        this.$Progress.start()
        this.$store.dispatch("allRecordReport", [this.pagination.current_page,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD'),this.manager_id]);
        this.$Progress.finish()
      },
      dateChange(){
        this.pagechange();
