...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      pagechange(){
        this.$Progress.start()
        this.$store.dispatch("allExpenditureReport", [this.pagination.current_page,this.luckydraw_id,this.kista_id,this.expenditure_type,moment(this.date.start).format('YYYY-MM-DD'),moment(this.date.end).format('YYYY-MM-DD'),this.manager_id]);
        this.$Progress.finish()
      },
      kistaChange(){
